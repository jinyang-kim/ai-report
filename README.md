...
```
kr-daily 커밋      → 빌드 → 1~2분 후 반영 (피드에 1건)
it-ai 커밋         → 빌드 → 1~2분 후 반영 (피드에 2건)
global-ui-ux 커밋  → 빌드 → 1~2분 후 반영 (피드에 3건)
       ↓
...
**작업을 독립적으로 커밋시키는 이유** — 한 작업이 실패해도 나머지 둘은 그대로 올라갑니다.
대신 오전 중에 사이트를 보면 아직 안 올라온 리포트가 있을 수 있습니다.
